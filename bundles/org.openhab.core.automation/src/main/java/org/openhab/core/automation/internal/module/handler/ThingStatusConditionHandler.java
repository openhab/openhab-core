...
public class ThingStatusConditionHandler extends BaseConditionModuleHandler implements EventSubscriber {
    ...
    public ThingStatusConditionHandler(Condition condition, String ruleUID, BundleContext bundleContext,
        ...
        this.thingUID = new ThingUID((String) module.getConfiguration().get(CFG_THING_UID));
    ...
    public void receive(Event event) {
        if ((event instanceof ThingAddedEvent addedEvent) && thingUID.toString().equals(addedEvent.getThing().UID)) {
            logger.info("Thing '{}' needed for rule '{}' added. Condition '{}' will now work.", thingUID, ruleUID,
                    module.getId());
            return;
        } else if ((event instanceof ThingRemovedEvent removedEvent)
                && thingUID.toString().equals(removedEvent.getThing().UID)) {
    ...
    public boolean isSatisfied(Map<String, Object> inputs) {
        String status = (String) module.getConfiguration().get(CFG_STATUS);
        String operator = (String) module.getConfiguration().get(CFG_OPERATOR);
        if (operator == null || status == null || thingUID == null) {
            logger.error("Module is not well configured: thingUID={}  operator={}  state = {} for rule {}", thingUID,
                    operator, status, ruleUID);
            return false;
        }
        ...
        if (thing == null) {
            ...
            switch (operator) {
                case "=":
                    return equalsToThingStatus(thing, status);
                case "!=":
                    return !equalsToThingStatus(thing, status);
                default:
    ...
    private boolean equalsToThingStatus(Thing thing, String state) {
        ThingStatus thingStatus = thing.getStatus();
        ThingStatus compareStatus = ThingStatus.valueOf(state);

        return thingStatus.equals(compareStatus);
    }
