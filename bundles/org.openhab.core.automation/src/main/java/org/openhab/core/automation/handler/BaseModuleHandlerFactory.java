...
public abstract class BaseModuleHandlerFactory implements ModuleHandlerFactory {
    ...
    public @Nullable ModuleHandler getHandler(Module module, String ruleUID) {
        String id = getModuleIdentifier(ruleUID, module.getTypeUID(), module.getId());
    ...
    public void ungetHandler(Module module, String ruleUID, ModuleHandler handler) {
        if (handlers.remove(getModuleIdentifier(ruleUID, module.getTypeUID(), module.getId()), handler)) {
            handler.dispose();
        }
    }

    protected String getModuleIdentifier(String ruleUid, String typeUID, String moduleId) {
        return ruleUid + "$" + typeUID + "$" + moduleId;
    }
}
