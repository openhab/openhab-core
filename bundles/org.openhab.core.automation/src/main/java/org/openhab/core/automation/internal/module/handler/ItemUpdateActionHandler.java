...
public class ItemUpdateActionHandler extends BaseActionModuleHandler {
    ...
    public ItemUpdateActionHandler(Action module, EventPublisher eventPublisher, ItemRegistry itemRegistry) {
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    String.format("Item with name '%s' not found in ItemRegistry.", itemName));
        }

        String state = (String) config.get(STATE);
        this.state = TypeParser.parseState(item.getAcceptedDataTypes(), state);
    }
    ...
    public @Nullable Map<String, Object> execute(Map<String, Object> inputs) {
        ...
        if (st instanceof State && item.getAcceptedDataTypes().contains(st.getClass())) {
            stateObj = (State) st;
        }
        if (stateObj != null) {
