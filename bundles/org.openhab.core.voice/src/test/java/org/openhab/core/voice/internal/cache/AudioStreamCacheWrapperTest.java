...
public class AudioStreamCacheWrapperTest {
    ...
    public void cacheWrapperStreamTest() throws IOException {
        ...
        when(mockedTTSResult.read(2, 1)).thenReturn(new byte[] { 3 });
        when(mockedTTSResult.read(3, 1)).thenReturn(new byte[0]);

        try (AudioStreamCacheWrapper audioStreamCacheWrapper = new AudioStreamCacheWrapper(mockedTTSResult,
        ...
        verify(mockedTTSResult, times(4)).read(anyInt(), anyInt());
        verify(mockedTTSResult).closeAudioStreamClient();
        verifyNoMoreInteractions(mockedTTSResult);
    }
    ...
    public void cacheWrapperStreamReadBunchTest() throws IOException {
        ...
        verify(mockedTTSResult, times(3)).read(anyInt(), anyInt());
        verify(mockedTTSResult).closeAudioStreamClient();
        verifyNoMoreInteractions(mockedTTSResult);
    }
