 ...
 */
@NonNullByDefault
public class JSonItem extends GenericItem {

    private static final List<Class<? extends State>> ACCEPTED_DATA_TYPES = List.of(JSonType.class, UnDefType.class);
    private static final List<Class<? extends Command>> ACCEPTED_COMMAND_TYPES = List.of(JSonType.class,
            RefreshType.class);

    public JSonItem(String name) {
        super(CoreItemFactory.JSON, name);
    }

    /**
     * Send a JSonType command to the item.
     *
     * @param command the command to be sent
     */
    public void send(JSonType command) {
     ...
     */
    public void send(JSonType command, @Nullable String source) {
    ...
    public void setTimeSeries(TimeSeries timeSeries) {
        if (timeSeries.getStates().allMatch(s -> s.state() instanceof JSonType)) {
