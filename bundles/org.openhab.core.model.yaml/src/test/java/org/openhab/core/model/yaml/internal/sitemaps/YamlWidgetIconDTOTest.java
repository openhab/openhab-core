...
public class YamlWidgetIconDTOTest {
    ...
    public void testIsValid() throws IOException {
        ...
        assertEquals("invalid type for \"name\" field", err.getFirst());
        err.clear();
        icon.name = List.of(0);
        assertFalse(icon.isValid(err, warn));
        assertEquals(1, err.size());
        assertEquals("invalid type for rule in \"name\" field", err.getFirst());
        err.clear();
        icon.name = "@switch";
        assertFalse(icon.isValid(err, warn));
        assertEquals(1, err.size());
        assertEquals(
                "invalid icon value \"@switch\" for \"name\" field; it must contain a maximum of 3 segments separated by a colon, each segment matching pattern [a-zA-Z0-9_][a-zA-Z0-9_-]*",
                err.getFirst());
        err.clear();
        ...
        assertEquals(1, err.size());
        assertEquals(
                "invalid icon value \"oh:classic:xxx:switch-on\" for \"name\" field; it must contain a maximum of 3 segments separated by a colon, each segment matching pattern [a-zA-Z0-9_][a-zA-Z0-9_-]*",
                err.getFirst());
        err.clear();
        icon.name = "switch";
        assertTrue(icon.isValid(err, warn));
        ...
        assertFalse(icon.isValid(err, warn));
        assertEquals(1, err.size());
        assertEquals("invalid type for \"color\" field", err.getFirst());
        err.clear();
        icon.color = List.of(0);
        assertFalse(icon.isValid(err, warn));
        assertEquals(1, err.size());
        assertEquals("invalid type for rule in \"color\" field", err.getFirst());
        err.clear();
        icon.color = "red";
        assertTrue(icon.isValid(err, warn));
        ...
        assertFalse(icon.isValid(err, warn));
        assertEquals(1, err.size());
        assertEquals("invalid rule in \"color\" field: \"value\" field missing while mandatory", err.getFirst());
        err.clear();
        rule1.value = "blue";
        assertTrue(icon.isValid(err, warn));
        rule1.operator = "<=";
        assertFalse(icon.isValid(err, warn));
        assertEquals(1, err.size());
        assertEquals("invalid rule in \"color\" field: \"argument\" field missing while mandatory in condition",
                err.getFirst());
        err.clear();
        rule1.argument = "50";
        assertTrue(icon.isValid(err, warn));
        ...
        assertFalse(icon.isValid(err, warn));
        assertEquals(1, err.size());
        assertEquals("invalid rule in \"name\" field: \"value\" field missing while mandatory", err.getFirst());
        err.clear();
        rule3.value = "@switch-on";
        assertFalse(icon.isValid(err, warn));
        assertEquals(1, err.size());
        assertEquals(
                "invalid rule in \"name\" field: invalid icon value \"@switch-on\" for \"value\" field; it must contain a maximum of 3 segments separated by a colon, each segment matching pattern [a-zA-Z0-9_][a-zA-Z0-9_-]*",
                err.getFirst());
        err.clear();
        ...
        assertEquals(1, err.size());
        assertEquals(
                "invalid rule in \"name\" field: invalid icon value \"oh:classic:xxx:switch-on\" for \"value\" field; it must contain a maximum of 3 segments separated by a colon, each segment matching pattern [a-zA-Z0-9_][a-zA-Z0-9_-]*",
                err.getFirst());
        err.clear();
        rule3.value = "switch-on";
        assertTrue(icon.isValid(err, warn));
        ...
        assertFalse(icon.isValid(err, warn));
        assertEquals(1, err.size());
        assertEquals("invalid rule in \"name\" field: \"argument\" field missing while mandatory in condition",
                err.getFirst());
        err.clear();
        rule3.argument = "50";
        assertTrue(icon.isValid(err, warn));
        ...
        assertEquals(1, err.size());
        assertEquals(
                "invalid rule in \"name\" field: invalid icon value \"@switch-off\" for \"value\" field; it must contain a maximum of 3 segments separated by a colon, each segment matching pattern [a-zA-Z0-9_][a-zA-Z0-9_-]*",
                err.getFirst());
        err.clear();
        ...
        assertEquals(1, err.size());
        assertEquals(
                "invalid rule in \"name\" field: invalid icon value \"oh:classic:xxx:switch-off\" for \"value\" field; it must contain a maximum of 3 segments separated by a colon, each segment matching pattern [a-zA-Z0-9_][a-zA-Z0-9_-]*",
                err.getFirst());
        err.clear();
        rule4.value = "switch-off";
        assertTrue(icon.isValid(err, warn));
