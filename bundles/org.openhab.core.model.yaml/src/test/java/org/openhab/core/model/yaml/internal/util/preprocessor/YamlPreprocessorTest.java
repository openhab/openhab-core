...
public class YamlPreprocessorTest {
    ...
    void packagingTest() throws IOException {
        Map<String, Object> data = (Map<String, Object>) YamlPreprocessor.load(SOURCE_PATH.resolve("packaging.yaml"));

        assertThat(YamlPreprocessor.getNestedValue(data, "things", "thing1", "thing1_property"), equalTo("thing1"));
        assertThat(YamlPreprocessor.getNestedValue(data, "things", "package1", "label"), equalTo("label1"));
        assertThat(YamlPreprocessor.getNestedValue(data, "things", "package2", "label"), equalTo("label2"));
    }

    private Map<String, Object> preprocessString(String data) {
        try {
            Path tempFile = java.nio.file.Files.createTempFile("yaml-test-", ".yaml");
            java.nio.file.Files.writeString(tempFile, data);
            try {
                Map<String, Object> result = (Map<String, Object>) YamlPreprocessor.load(tempFile);
                return result;
            } finally {
