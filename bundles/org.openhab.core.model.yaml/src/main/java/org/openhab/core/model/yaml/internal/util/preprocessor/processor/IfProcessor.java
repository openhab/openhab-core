...
public class IfProcessor implements PlaceholderProcessor<IfPlaceholder> {
    ...
    public IfProcessor(PreprocessorContext context) {
    ...
    @Override
    public @Nullable Object process(IfPlaceholder ifPlaceholder) {
        ...
        for (Branch branch : logic.branches()) {
            Object evaluated = switch (branch.condition()) {
                ...
                case String s -> StringInterpolator.evaluateExpression(s, context.getVariables(),
                        context.getLogger().getLogSession(), ifPlaceholder.sourceLocation());
                default -> branch.condition();
            };

            if (StringInterpolator.isTruthy(evaluated)) {
    ...
    private @Nullable Logic parseFromListDefinition(List<?> list, String sourceLocation) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (!(item instanceof Map<?, ?> map)) {
                context.getLogger().warn("{} !if list item is not a mapping and will be ignored: {}.", sourceLocation,
                        item);
                continue;
            }
            ...
            if (map.containsKey("else")) {
                ...
                if (map.containsKey("if") || map.containsKey("elseif") || map.containsKey("then")) {
                    context.getLogger().warn(
                            "{} !if sequence item at index {} should only contain 'else'. Other keys ignored.",
                            sourceLocation, i);
                }
                if (i < list.size() - 1) {
                    context.getLogger().warn("{} !if list has unreachable branches after the 'else' block.",
                            sourceLocation);
                }
        ...
        if (branches.isEmpty()) {
            context.getLogger().warn("{} !if sequence has no valid branches.", sourceLocation);
        }
    ...
    private @Nullable Branch createBranch(Map<?, ?> map, boolean isElseIf, boolean isList, String sourceLocation) {
        ...
        if (!hasValueKey || !hasConditionKey) {
            ...
            context.getLogger().warn("{} !if {}{}. {}", sourceLocation, prefix, reason, suffix);
            return null;
        }
