...
public class YamlWidgetIconDTO {
    ...
    public boolean isValid(@NonNull List<@NonNull String> errors, @NonNull List<@NonNull String> warnings) {
        ...
        if (name instanceof String nameStr) {
            ...
            ok &= rule.isValid(errors, warnings);
            if (rule.value == null) {
                addToList(errors, "\"value\" field missing while mandatory in icon name rule");
                ok = false;
            } else if (!YamlElementUtils.isValidIcon(rule.value)) {
                addToList(errors,
                        "invalid value \"%s\" for \"value\" field in icon name rule; it must contain a maximum of 3 segments separated by a colon, each segment matching pattern [a-zA-Z0-9_][a-zA-Z0-9_-]*"
            ...
            for (Object r : rules) {
                if (r instanceof YamlRuleWithAndConditionsDTO rule) {
                    ...
                    addToList(errors, "invalid type for icon name rule");
                    ok = false;
                }
            }
            ruleErrors.forEach(error -> {
                addToList(errors, "icon name: %s".formatted(error));
            });
            ruleWarnings.forEach(warning -> {
                addToList(warnings, "icon name: %s".formatted(warning));
            });
        } else if (name != null && !(name instanceof String)) {
            addToList(errors, "icon name: invalid type");
            ok = false;
        }
        ...
        if (color instanceof YamlRuleWithAndConditionsDTO rule) {
            ok &= rule.isValid(errors, warnings);
            if (rule.value == null) {
                addToList(errors, "\"value\" field missing while mandatory in icon color rule");
                ok = false;
            }
            ...
            for (Object r : rules) {
                if (r instanceof YamlRuleWithAndConditionsDTO rule) {
                    ...
                    addToList(errors, "invalid type for icon color rule");
                    ok = false;
                }
            }
            ruleErrors.forEach(error -> {
            ...
            addToList(errors, "icon color: invalid type");
            ok = false;
        }
