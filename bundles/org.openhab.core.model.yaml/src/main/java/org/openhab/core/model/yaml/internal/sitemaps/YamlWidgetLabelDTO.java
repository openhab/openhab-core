...
public class YamlWidgetLabelDTO {
    ...
    public boolean isValid(@NonNull List<@NonNull String> errors, @NonNull List<@NonNull String> warnings) {
        ...
        if (labelColor instanceof YamlRuleWithAndConditionsDTO rule) {
            ...
            if (rule.value == null) {
                addToList(errors, "\"value\" field missing while mandatory in label color rule");
                ok = false;
            }
            ...
            for (Object r : rules) {
                if (r instanceof YamlRuleWithAndConditionsDTO rule) {
                    ...
                    addToList(errors, "invalid type for label color rule");
                    ok = false;
                }
            }
            ...
            addToList(errors, "label color: invalid type");
            ok = false;
        }
        ...
        if (valueColor instanceof YamlRuleWithAndConditionsDTO rule) {
            ok &= rule.isValid(errors, warnings);
            if (rule.value == null) {
                addToList(errors, "\"value\" field missing while mandatory in value color rule");
                ok = false;
            }
            ...
            for (Object r : rules) {
                if (r instanceof YamlRuleWithAndConditionsDTO rrule) {
                    ...
                    addToList(errors, "invalid type for value color rule");
                    ok = false;
                }
            }
            ruleErrors.forEach(error -> {
            ...
            addToList(errors, "value color: invalid type");
            ok = false;
        }
