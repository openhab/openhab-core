...
public class ScriptItemRefresher implements ItemRegistryChangeListener {
    ...
    protected void addActionService(ActionService actionService) {
        logger.info("Script action added => scripts are going to be refreshed");
        scheduleScriptRefresh();
    }

    protected void removeActionService(ActionService actionService) {
        logger.info("Script action removed => scripts are going to be refreshed");
        scheduleScriptRefresh();
    }

    @Override
    public void added(Item element) {
        logger.info("Item \"{}\" added => scripts are going to be refreshed", element.getName());
        scheduleScriptRefresh();
    }

    @Override
    public void removed(Item element) {
        logger.info("Item \"{}\" removed => scripts are going to be refreshed", element.getName());
        scheduleScriptRefresh();
    }
    ...
    public void allItemsChanged(Collection<String> oldItemNames) {
        logger.info("All items changed => scripts are going to be refreshed");
        scheduleScriptRefresh();
    }
