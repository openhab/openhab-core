...
public class ChannelTypeResourceTest {
    ...
    public void setup() {
        ...
        channelTypeResource.localeService = Mockito.mock(LocaleService.class);
        channelTypeResource.channelTypeRegistry = (channelTypeRegistry);
        channelTypeResource.profileTypeRegistry = (profileTypeRegistry);
    }
    ...
    public void returnLinkableItemTypesForTriggerChannelType() throws IOException {
        ...
        Stream<String> response = channelTypeResource.getLinkableItemTypes(uid.getAsString());

        verify(channelTypeRegistry).getChannelType(uid);
        verify(profileTypeRegistry).getProfileTypes();
        assertThat(response.collect(Collectors.toList()), IsCollectionContaining.hasItems("Switch", "Dimmer"));
    }
